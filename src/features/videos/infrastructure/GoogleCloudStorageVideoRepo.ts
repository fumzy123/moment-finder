...
export class GoogleCloudStorageVideoRepo implements IVideoRepo {
    ...
    // Video Screenshot
    async uploadVideoScreenshot(videoId: string, screenshot: File): Promise<VideoScreenshot> {
        ...
        const gcsFile = this.uploadedVideosBucket.file(videoScreenShotstoragePath);
        ...
        const uploadedScreenshot: VideoScreenshot = {
            ...
            url: await this.generateSignedUrlforMediaFile(videoScreenShotstoragePath),
            videoId,
            bucket: this.uploadedVideosBucket.name
        }
        return uploadedScreenshot;
    }
    async getVideoScreenshot(videoId: Video['id'], screenshotId: VideoScreenshot['id']): Promise<VideoScreenshot> {
    ...
    async getVideoScreenshots(videoId: string): Promise<VideoScreenshot[]> {
        if (!videoId) {
    ...
    // Signed URL
    async generateSignedUrlforMediaFile(fileName: string): Promise<string> {
        if (!fileName) {
